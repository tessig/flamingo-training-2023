...
var (
	errorCounter = stats.Int64("openweather/api/error_count", "number of API errors", stats.UnitDimensionless)
)
...
func (c *APIClient) request(ctx context.Context, method, path string, body io.Reader) (*http.Response, error) {
	ctx, span := trace.StartSpan(ctx, "openweather/api/request")
	defer span.End()
	...
	if err != nil {
		...
		c.logger.WithContext(ctx).
			...
			Error(fmt.Errorf("error on openweather api call: %w", err))
	}
